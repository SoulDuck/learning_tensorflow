...
def get_imagenet_data(imagenet_folder,img_num) :
    files = sorted(listdir(imagenet_folder))
    ...
    f = open("ILSVRC2012_validation_ground_truth.txt", 'r')
    while True:
        line = f.readline()
        if not line: break
    ...
    for f,l in zip(files,labels) :
        ...
        img = Image.open(imagenet_folder+"/"+f).resize((224,224))
        ...
        label = np.zeros((1000,), dtype=np.int)
        label[labels[l]-1] = 1
